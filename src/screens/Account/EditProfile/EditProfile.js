...
class EditProfile extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const {
      successGetProfileVersion,
      errorGetProfileVersion,
      successUpdateProfileVersion,
      errorUpdateProfileVersion,
      successAllParameterVersion,
      errorAllParamaterVersion,
      successGetStateListVersion,
      errorGetStateListVersion,
      successGetCityListVersion,
      errorGetCityListVersion,
    } = nextProps;
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    if (
      ...
      this.setState({
        name: getProfileData.data.full_name
          ? getProfileData.data.full_name
          : '',
        email: getProfileData.data.email_id ? getProfileData.data.email_id : '',
        mobileNo: getProfileData.data.mobile_number
          ? getProfileData.data.mobile_number
          : '',
        designation: getProfileData.data.designation
          ? getProfileData.data.designation
          : '',
        companyName: getProfileData.data.organisation
          ? getProfileData.data.organisation
          : '',
        ...
        dobSelected: getProfileData.data.birthday
          ? getProfileData.data.birthday
          : '',
        anniversaryDate: getProfileData.data.anniversary_date
          ? getProfileData.data.anniversary_date
          : '',
      });

      let c = allParameterData.country_data.filter(
        i => i.id == getProfileData.data.country_id,
      );
    ...
    if (
      this.state.successUpdateProfileVersion >
      prevState.successUpdateProfileVersion
    ) {
      this.showToast(this.props.errorMsg, 'success');
    }
    if (
      this.state.errorUpdateProfileVersion > prevState.errorUpdateProfileVersion
    ) {
      this.showToast(this.props.errorMsg, 'danger');
    }
    ...
    if (
      this.state.successGetStateListVersion >
      prevState.successGetStateListVersion
    ) {
      if (isFirstTime) {
        let st = stateList.states.filter(
          i => i.id == getProfileData.data.state_id,
        );
    ...
    if (
      this.state.successGetCityListVersion > prevState.successGetCityListVersion
    ) {
      if (isFirstTime) {
        let abc = cityList.cities.filter(
          i => i.id == getProfileData.data.city_id,
        );
              ...
              <TouchableWithoutFeedback
                ...
                <>
                  ...
                  {this.state.countryData && this.state.countryData.length > 0 && (
                    <FlatList
                      data={
                        this.state.enteredCountry.length
              ...
              <TouchableWithoutFeedback
                ...
                <>
                  ...
                  {this.state.stateData && this.state.stateData.length > 0 && (
                    <FlatList
                      data={
                        this.state.enteredState.length
