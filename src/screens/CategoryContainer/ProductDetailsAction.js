    ...
    axios
      ...
      .then(response => {
        if (response.data.ack === '1') {
          dispatch(onSuccess(response.data, PRODUCT_DETAILS_DATA_SUCCESS));
        } else {
          dispatch(onFailure(response.data.msg, PRODUCT_DETAILS_DATA_ERROR));
        }
      })
...
export function addToCartFromDetails(data) {
  console.log('addToCartFromDetails==>', data);
  return dispatch => {
